...
class Model_1():
    def __init__(self):
        ...
        self.train_order = np.argsort(self.qids_train)
        self.test_order = np.argsort(self.qids_test)

        self.X_train = self.X_train[self.train_order]
        self.y_train = self.y_train[self.train_order]
        self.qids_train = self.qids_train[self.train_order]

        self.X_test = self.X_test[self.test_order]
        self.y_test = self.y_test[self.test_order]
        self.qids_test = self.qids_test[self.test_order]

        print("Cheking quick usability")

        check_qids(self.qids_train)



    def qid_validation(self):
        valid_qids = self.stock_df.groupby('qid')['relevance'].nunique()
        print((valid_qids >= 2).mean())
    ...
    def get_hyperparameter(self, save_path="../saved_models/lambdamart_best_model.pkl"):
        """
        ...
        self.X_train, X_valid, self.y_train, y_valid, self.qids_train, qids_valid = train_test_split(
            self.X_train, self.y_train, self.qids_train, test_size=0.25, random_state=42
        )
        ...
        valid_order = np.argsort(qids_valid)
        X_valid = X_valid[valid_order]
        y_valid = y_valid[valid_order]
        qids_valid = qids_valid[valid_order]
        ...
        for i in range(10):
            ...
            score = metric.evaluate(y_valid, preds)
